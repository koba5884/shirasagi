...
RSpec.describe Board::Post, type: :model, dbscope: :example do
  ...
  describe "mode and permit_comment field validations" do
    it { expect(build(:board_topic,   mode: nil)          ).not_to be_valid }
    it { expect(build(:board_comment, mode: nil)          ).to     be_valid }
    it { expect(build(:board_topic,   permit_comment: nil)).not_to be_valid }
    it { expect(build(:board_comment, permit_comment: nil)).to     be_valid }
  end
  ...
  describe "#root_post" do
    ...
    context "topic" do
      let(:post) { build(:board_topic) }
      it { is_expected.to eq post }
    end

    context "comment" do
      let(:post) { build(:board_comment) }
      it { is_expected.to eq post.parent }
    end

    context "comment to comment" do
      let(:post) { build(:board_comment_to_comment) }
      it { is_expected.to eq post.parent.parent }
    end
  end

  describe "#comment?" do
    it { expect(build(:board_topic).comment?).to be_falsy }
    it { expect(build(:board_comment).comment?).to be_truthy }
  end
  ...
  describe "#set_topic_id" do
    ...
    context "topic" do
      let(:post) { create :board_topic }
      it { is_expected.to be_nil }
    end

    context "comment" do
      let(:post) { create :board_comment }
      it { is_expected.to eq post.parent.id }
      it { is_expected.to eq post.root_post.id }
    end

    context "comment to comment" do
      let(:post) { create :board_comment_to_comment }
      it { is_expected.to eq post.root_post.id }
    end
  end
